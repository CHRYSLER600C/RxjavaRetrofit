 ...
 * @author zihao
 */
public class PickerScroller extends View {
    /**
     * 控件宽度
     */
     ...
     * 设置数据
     */
    private ArrayList<NameValue> dataList = new ArrayList<NameValue>();
    /**
     * 按下的坐标
     */
    private int downY;
    ...
    public PickerScroller(Context context, AttributeSet attrs, int defStyle) {
    ...
    public PickerScroller(Context context, AttributeSet attrs) {
        super(context, attrs);
        // TODO Auto-generated constructor stub
        init(context, attrs);
        initData();
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                if (onSelectListener != null)
                    onSelectListener.endSelect(item.id, item.itemName);
                break;
            }
        }
    }
    ...
    private void initData() {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            itmItemObject.id = i;
            itmItemObject.itemName = dataList.get(i).name;
            itmItemObject.itemValue = dataList.get(i).value;
            itmItemObject.x = 0;
        ...
        if (move > 0) {
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).isSelected()) {
                    ...
                    if (onSelectListener != null)
                        onSelectListener.endSelect(itemList.get(i).id, itemList.get(i).itemName);
                    break;
                }
            }
        } else {
    ...
    private void onSelectListener() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                onSelectListener.selecting(item.id, item.itemName);
            }
        }
    }
     ...
     * @param data
     */
    public void setData(ArrayList<NameValue> data) {
        this.dataList = data;
        initData();
    }
    ...
    public NameValue getSelectedNameValue() {
        for (ItemObject item : itemList) {
            if (item.isSelected())
                return new NameValue(item.itemName, item.itemValue);
        }
     ...
     * 获取NameValue键值对
     */
    public NameValue getNameValueById(int id) {
    ...
    public String getItemText(int index) {
        if (itemList == null)
            return "";
        return itemList.get(index).itemName;
    }
    ...
    private class ItemObject {
        ...
        public void drawSelf(Canvas canvas) {
            ...
            // 绘制内容
            canvas.drawText(itemName, x + controlWidth / 2 - textRect.width() / 2,
